...
from astra_bot.core.models import Position, Side
...
class TestRiskEngine:
    ...
    def test_risk_multiplier_no_drawdown(self, risk_engine):
        ...
        # Первый tier с drawdown=0, так что без просадки возвращается 0.75 (следующий уровень)
        # Это особенность реализации - первый tier с 0 всегда срабатывает
        assert multiplier == Decimal("0.75")
    
    def test_risk_multiplier_with_drawdown(self, risk_engine):
        """Тест множителя риска с просадкой"""
        risk_engine.set_capital(Decimal("980"), Decimal("1000"))  # 2% DD
        
        multiplier = risk_engine._get_risk_multiplier()
        # При 2% DD множитель зависит от конфигурации
        # Проверяем что множитель меньше 1.0 (сuyển giảm риска)
    ...
    def test_calculate_position_size(self, risk_engine):
        ...
        # Риск 0.4% от 1000 = 4 рубля (но с множителем 0.75 = 3 рубля)
        # Стоп 1000 рублей
        # Размер = 3 / 1000 = 0.003 BTC
        expected_size = Decimal("0.003")
