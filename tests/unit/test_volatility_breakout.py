...
async def test_volatility_breakout_long():
    strategy = VolatilityBreakoutStrategy(
        VolatilityBreakoutConfig(squeeze_ratio=0.8, squeeze_min_bars=10, volume_mult=1.2, roc_period=14)
    )
    candles = []
    base_ts = 1000000
    ...
    candles.append(make_candle(100.0, 103.5, 100.0, 103.0, volume=200.0, timestamp_ms=base_ts + 55 * 3600000))

    signal = await strategy.evaluate("BTC-USDT", candles, current_price=103.0)
    assert signal is not None
    assert signal.direction == TradeDirection.LONG
    assert float(signal.entry_price) == 103.0
