...
def gen_candles(n: int = 230, tf: str = "5m") -> list[models.Candle]:
    ...
    Бэклог A5: входы — только по закрытым барам. Сигнальный узор
    (откат + бычья свеча) стоит на ПОСЛЕДНЕМ ЗАКРЫТОМ баре (n-2);
    последняя свеча (n-1) — «формирующаяся»: в живой работе её не
    видят стратегии (pipeline._closed_entry_candles), поэтому и в
    тестовом фиде сигнал должен срабатывать на n-2.
    """
    out = []
    price = 100.0
    for i in range(n):
        ...
        if i < n - 5:
            ...
            c = o + 0.025  # формирующийся бар — мягкое продолжение
        h = max(o, c) + 0.02
        lo = min(o, c) - 0.02
        out.append(
            models.Candle(
