...
from .events import EventType, EventBus, emit
...
class SystemState:
    ...
    risk_state_changed_at: Optional[datetime] = None
    ...
    def get_risk_multiplier(self) -> float:
        ...
        for tier in config.risk.drawdown_adaptation:
            if dd >= float(tier["drawdown"]):
                continue
            return float(tier["risk_multiplier"])
        
        return 0.0  # Если просадка выше всех порогов
