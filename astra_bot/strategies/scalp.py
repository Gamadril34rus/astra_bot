"""
Scalp-стратегия для частых мелких сделок.

Цель — генерировать много сделок на 15m таймфрейме, чтобы ML быстрее
накапливал реальные уроки. Риск жёстко ограничен коротким стопом и малым
R:R (компенсируется высоким win-rate). Это не «всё подряд»: вход только
по тренду (EMA-стек), на откате к EMA21, с RSI в нейтральной зоне и
подтверждающей свечой.
"""
...
class ScalpStrategy(BaseStrategy):
    ...
    def __init__(self, config: ScalpConfig | None = None):
        super().__init__(config or ScalpConfig())
    ...
    async def evaluate(
        ...
        dist = abs(price - e21) / price
        if dist > c.pullback_pct:
            return None
        ...
        closeness = 1.0 - min(1.0, dist / c.pullback_pct)
        confidence = min(0.85, 0.5 + 0.25 * closeness + 0.1 * (rsi_val - 50) / 10.0)
    ...
    def calculate_stop_loss(self, entry_price, candles, atr=None):
        return entry_price * Decimal("0.994")
