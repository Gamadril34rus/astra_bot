...
class MeanReversionStrategy(BaseStrategy):
    ...
    async def evaluate(
        ...
        if current_price <= bb["lower"] or z_score < -self.config.zscore_threshold:
            if (not self.config.require_bb_touch and z_score < -self.config.zscore_threshold) or (self.config.require_bb_touch and current_price <= bb["lower"]):
                pass
            else:
                return None

            if (self.config.require_rsi_confirm and rsi and rsi < self.config.rsi_oversold) or not self.config.require_rsi_confirm:
        ...
        # SHORT: цена у верхней BB + RSI overbought + высокий z-score
            ...
            if (not self.config.require_bb_touch and z_score > self.config.zscore_threshold) or (self.config.require_bb_touch and current_price >= bb["upper"]):
                pass
            else:
                return None

            if (self.config.require_rsi_confirm and rsi and rsi > self.config.rsi_overbought) or not self.config.require_rsi_confirm:
