...
Целевой universe — **10 ликвидных USDT-пар** (BTC ETH SOL XRP DOGE ADA AVAX LINK DOT TRX), как в `SystemConfig.instruments`. Расширение только через research→validation→champion. Перед торговым циклом список сверяется с актуальными перпетуал-контрактами BingX (USDT-M). Бот торгует ТОЛЬКО бессрочными фьючерсами — спотового контура нет.
...
| `ASTRA_LEVERAGE_MAX` | потолок плеча (по умолчанию 100) |
