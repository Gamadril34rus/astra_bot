...
- плата за плечо (0.04%/сутки на заёмную часть) растёт вместе с
  плечом и всегда в PnL — долго держать 100x заведомо дорого;
...
- **Нетто-безубыток**: после +0.8R (с плечом — после +0.6R) стоп
  переносится в точку, покрывающую комиссии обеих сторон и плату за
  плечо. «Безубыток в сырой вход» — это маленький гарантированный минус.
...
- **Плата за плечо** (фандинг) начисляется при каждом закрытии на
  заёмную часть объёма пропорционально барам удержания — детерминированно
  (реплей даёт идентичные сделки). Забыть её нельзя: тест
  `test_trade_never_forgets_commission`.
