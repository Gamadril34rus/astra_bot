...
NewsAPI даёт исторический поиск до 5 лет на платных планах. Бесплатный Developer-план предназначен для разработки/тестирования, поэтому его нельзя использовать как постоянный рабочий источник. 
...
По монетам, появившимся менее 5 лет назад, используется вся доступная история, а не выдуманные свечи. Никакой магии, только унылая статистика.
...
OKX Demo API требует `x-simulated-trading: 1`; worker запускается только с `OKX_DEMO=1`. citeturn607296search0turn607296search10
