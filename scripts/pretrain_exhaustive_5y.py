#!/usr/bin/env python3
"""Research-first historical learning pass.

The primary objective is market discovery, not trade count. The pass records
observable conditions, market events and multi-horizon consequences before
running self-play. No exchange orders are ever created here.
"""
...
from astra_bot.ml.research_engine import observe, aggregate
...
async def fetch_history() -> dict[str, list[models.Candle]]:
    ...
    try:
        result: dict[str, list[models.Candle]] = {}
        for symbol in TRADING_UNIVERSE:
            try:
                ...
                print(f"History {symbol}: failed: {type(exc).__name__}: {exc}", flush=True)
        return result
    finally:
        await client.close()
...
def _lesson_from_signal(signal, strategy, window, future, cross, news):
    ...
    for bar in future[:48]:
        if direction == "long":
            if bar.low <= stop:
                exit_price, exit_time = stop, bar.open_time
                break
            if bar.high >= take:
                exit_price, exit_time = take, bar.open_time
                break
        else:
            if bar.high >= stop:
                exit_price, exit_time = stop, bar.open_time
        ...
        if future:
            ...
            exit_price, exit_time = last.close, last.open_time
    gross = (exit_price - entry) if direction == "long" else (entry - exit_price)
    pnl = gross - entry * Decimal("0.0005")
    ...
    return {
        ...
        "influencing_factor": sp._influencing_factor(features, outcome),
        ...
        "training_phase": "max_available_history_research_first_walk_forward",
...
async def run(args) -> int:
    ...
    usable = {s: b for s, b in history.items() if len(b) >= 250}
    if len(usable) < 10:
    ...
    # Research pass: sample independent market states before generating trades.
    for step, ts in enumerate(timestamps):
        if step < 50 or step % RESEARCH_SAMPLE_EVERY:
            continue
        for symbol, bars in usable.items():
            ...
            try:
                window = bars[: idx + 1]
                features = compute_market_features(window, timeframe=window[-1].timeframe)
                obs = observe(symbol, window[-1].timeframe, bars, idx, features_before={k: float(v) for k, v in features.items()})
                if obs:
                    observations.append(obs)
            except Exception as exc:
                print(f"Research {symbol} @ {ts}: {type(exc).__name__}: {exc}", flush=True)

    research_path = Path("models/research_observations.jsonl")
    research_path.parent.mkdir(parents=True, exist_ok=True)
    with research_path.open("w", encoding="utf-8") as f:
        for obs in observations:
            f.write(json.dumps(obs.to_dict(), ensure_ascii=False) + "\n")
        for hypothesis in aggregate(observations):
            f.write(json.dumps(hypothesis, ensure_ascii=False) + "\n")

    # Self-play remains useful, but is explicitly secondary to research.
    for step, ts in enumerate(timestamps):
        if step < 250:
            continue
    ...
    print(json.dumps({
        ...
        "memory_research": len(memory.data.get("research", {})),
...
async def main() -> int:
    ...
    p.add_argument("--max-lessons", type=int, default=500_000)
    p.add_argument("--min-samples", type=int, default=2_000)
