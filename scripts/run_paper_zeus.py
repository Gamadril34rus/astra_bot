...
async def observe_zeus(* , bingx: BingXClient, zeus: ZeusWedgeRetestStrategy, journal: ZeusTradeLog, symbol: str) -> None:
    try:
        ...
        try:
            ...
            if k15 and len(k15) >= LTF_LOOKBACK:
                ...
                if avg_r > 0 and last_r >= avg_r * LTF_IMPULSE_RANGE_MULT:
                    ...
                    journal.ltf_impulse(
                        symbol=symbol, timeframe="15m", note=note,
...
async def amain(args: argparse.Namespace) -> int:
    ...
    bingx = BingXClient({"api_key": api_key, "api_secret": api_secret, "enabled": True, "rate_limit_qps": 5})
    ...
    async def one_cycle() -> None:
        ...
        known_ids = sync_journal_from_broker(
            engine=engine, journal=journal, before=before,
            trades_path=trades_path, known_trade_ids=known_ids,
        )
