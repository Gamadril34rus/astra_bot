...
def generate_daily_report() -> str:
    ...
    drawdown_status = "🟢 Норма" if float(state.current_drawdown) < 3 else \
                      "🟡 Внимание" if float(state.current_drawdown) < 5 else \
                      "🔴 Высокая" if float(state.current_drawdown) < 8 else "🚨 Критическая"
    ...
    # Profit Factor (приблизительный)
    if losses > 0 and wins > 0:
        total_wins_pnl = sum(1 for _ in range(wins))  # упрощённо
        total_losses_pnl = sum(1 for _ in range(losses))
        # В реальности нужно считать из базы данных
        pf = 1.5  # placeholder
    else:
        pf = 0.0
