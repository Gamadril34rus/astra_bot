...
### Эпоха-2: хард-ресет paper-счёта 08.09.2026 (PR #63)
- Причина: стратегии эпохи-1 слили −4123 на 367 сделках (просадка 10.25% → честный EMERGENCY, не баг); статистика EV не копилась из-за бага записи (уже пофикшен: stats_store.record теперь пишется всегда). Старые убытки признаны нерепрезентативными.
- Сброс: paper_trades.jsonl очищен (история в git), paper_positions.json → старт 36098.87. Маркер: models/learning_epoch.json (epoch 2, since 2026-09-08).
- Временно (первые ~2 недели эпохи-2): потолок плеча paper-контура 3x (ASTRA_LEVERAGE_MAX, дефолт в scripts/run_bot.py) — чистые данные для калибровки EV/confidence. Поднять: ASTRA_LEVERAGE_MAX=100.
- Громкая проверка загрузки стратегий: движок логирует «Загружено стратегий N/17», падение паттернов — ERROR (было: тихий warning + пустой список).
- Правило: больше историю не вайпать — только миграция отдельных битых записей. Отсчёт готовности (30 дней / 200 сделок) — с 2026-09-08.
