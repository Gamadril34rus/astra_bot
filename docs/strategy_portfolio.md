...
BTC/ETH/SOL одной и той же стратегией (данные ETH/SOL для исторической
валидации в песочнице недоступны — стратегия символ-агностична, но
прогон выполнен на BTC).
...
```bash
python scripts/strategy_lab.py            # протокол IS/OOS/история + портфель
python scripts/research_free_strategies.py  # полный список 16+ правил
```
